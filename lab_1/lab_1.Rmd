...
subtitle: ALberto Daguerre, Joaquín Moreno y Pablo Rivero  
...
Tal y como se ve en el gráfico, dependió mucho en que clase viajaba cada pasajero.
Ajustándonos únicamente al gráfico sin saber los resultados al milímetro, tenemos como conclusión,
que de los viajeros de la clase 1, casi falleció un cuarto de la gente. Los que viajaban en la clase 2,
podríamos decir que murireron algo mas de la mitad de los pasajeros. Y por último, sobre los viajeros de la clase 3, podríamos decir que solo sobrevivió el un poco más del veinte porcierto de las personas que viajaban en dicha clase.
...
Como podemos observar en el gráfico de barras de la parte superior, el porcentaje de mujeres que sobrevivieron es mucho mayor al de los hombres. En el case de las mujeres alrededor de un 75% de las mujeres que embarcaron consiguieron sobrevivir a la catástrofe, mientras que el de hombres que sobrevivieron apenas llega al 20%. Esto puede deberse a que los hombres cedieron su posibilidad de sobrevivir a sus esposas o hijas.
...
```{r}
count_by_age = function(data, lower, upper){
  data = data[data$Age >= lower & data$Age <= upper, ]
  new_data = data[is.na(data$Age) == FALSE, ]
  survived = sum(new_data$Survived)
  survived
}

intervalos = seq(0, 80, by=10)
vector = c()
for (i in intervalos){
  vector = c(vector, count_by_age(data, i, i+10))
}
vector
table(vector, intervalos)
```
...
```{r}
survived = data
hist(x = data$Age, col = "Green", ylim= c(0,300), main = "Histograma de Edad", xlab = "Edad", ylab = "Frecuencia")

hist(x = data$Age, col = "Green", ylim= c(0,300), main = "Histograma de Edad", xlab = "Edad", ylab = "Frecuencia", breaks = 12)
```

*Tus explicaciones, aquí*

2. El gráfico obtenido muestra que la mayoria de pasajeros que viajaban en el titanic se encontraba en el grupo de edad de entre 20-40 años, siendo el grupo de edad con mas pasajeros el de 20-25 años contando con unas 150 ocurrencias.

3. El argumento breaks modifica el numero de barras que se mostrarán en el gráfico de forma que cuanto mayor sea el valor que le demos al argumento, mayor será el numero de barras que se muestren.
...
```{r}
group_age = seq(-10, 70, by=10)
vector = c()
for (i in group_age){
  vector = c(vector, i+ 10)
}
intervalos_edades = cut(data$Age, breaks = vector, na.rm = TRUE)
tabla = table(intervalos_edades)
which.max(tabla)
```

```{r}
var(data$Age, na.rm = TRUE)# años al cuadrado
sd(data$Age, na.rm = TRUE)#años
```
...
```{r boxplot}
boxplot(Age ~ Pclass, data = data, horizontal = TRUE, col = "Green")
```
...
Podemos observar que el grupo de edad de los pasajeros que viajaron en primera es mucho mas amplio (contiene mas variedad de edades) que los grupos que viajan en tercera o segunda. Tambien podemos observar que la media de edad de los pasajeros que viajaban en primera rondaba los 40 años, mientras que la de los viajeros que contrataron un pase de tercera o segunda ronda los 25 años. Esto puede deberse a que raramente encontramos a alguien joven con una capacidad economica suficiente como para poder permitirse un viaje en primera clase en un crucero, probablemente porque han empezado a trabajar hace escasos años y estan atosigados por hipotecas de coche, casa, etc.
...
intervalos_edades = cut(data$Age, breaks = vector, na.rm = TRUE)
tabla = table(intervalos_edades)
proporcion_supervivientes = prop.table(tabla)
barplot(proporcion_supervivientes, legend.text=c('Supervivientes'), col=c('Blue'))
```

*Tus conclusiones, aquí*

Podemos observar que el grupo de edad con mayor numero de supervivientes fue el de los 20-30 años, esto es entendible ya que como observamos en ejercicios anteriores el rango de edad que agrupaba mas pasajeros era este mismo. En general el numero de supervivientes por edad esta directamente relacionado con el numero de pasajeros de su rango de edad.
